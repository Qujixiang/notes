...
/**
 * @brief 加载二进制文件
 *        设置bfd对象的文件名称，文件类型，机器架构，位数，程序入口地址
 * @param fname 文件路径
 * @param bin 二进制文件对象指针，用于设置解析完毕的二进制文件信息
 * @param type 二进制文件类型
 ...
 * @brief 获取文件的符号表
 * 
 * @param bfd_h 文件的bfd指针
 * @param bin 将文件的符号表加载到bin对象中
 * @return int 0表示成功，-1表示失败
 */
static int load_symbols_bfd(bfd *bfd_h, Binary *bin) {
    int ret = 0;
...
/**
 * @brief 获取文件的所有节
 *        获取节的标志，类型，虚拟内存地址，大小，名称
 * @param bfd_h 文件的bfd指针
 * @param bin 将文件的节加载到bin对象中
 * @return int 0表示成功，-1表示失败
 */
static int load_sections_bfd(bfd* bfd_h, Binary *bin) {
    flagword bfd_flags;
